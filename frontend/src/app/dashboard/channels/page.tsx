...
export default function ChannelsPage() {
  ...
  const loadAccounts = useCallback(async () => {
    ...
    try {
      ...
      console.error('[DEBUG] Failed to load accounts:', err);
  ...
  const repairChannel = async (id: string) => {
    ...
    try {
      ...
      await loadAccounts();
      alert('Connection repaired successfully! Meta webhooks have been re-subscribed.');
    } catch (error) {
      alert(error instanceof Error ? error.message : 'Failed to repair connection');
    } finally {
      setLoading(false);
    }
  };
                        ...
                        {connected && (
                          <div className="flex gap-2">
                            <button
                              ...
                              disabled={loading}
                              className="px-4 py-2 rounded-xl border border-blue-200 text-blue-600 text-sm font-medium hover:bg-blue-50 transition-colors disabled:opacity-50"
                            >
                              Repair
                            </button>
                            <button
