          ...
          <div className="h-full flex flex-col bg-white">
              ...
              <div className="flex items-center justify-between gap-6">
                ...
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    >
                      <div className="w-10 h-10 rounded-full bg-gray-200 flex-shrink-0 overflow-hidden flex items-center justify-center border border-gray-100 shadow-sm">
                        {conversation.customer?.avatarUrl ? (
                          <img src={conversation.customer.avatarUrl} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <span className="text-sm font-bold text-gray-600">{customerInitial}</span>
                        )}
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <button
                            ...
                            className={`p-1 rounded-md hover:bg-gray-100 text-gray-400 hover:text-blue-600 transition-colors ${syncingProfile ? 'animate-spin text-blue-600' : ''}`}
                ...
                {allMessages.length === 0 ? (
                  ...
                  allMessages.map((message) => {
                    ...
                    const senderName = isStaff 
                      ? (staff.find(s => s.id === message.senderId)?.name || 'Staff')
                      : (conversation.customer?.name || 'Customer');
                    
                    return (
                      <div
                        key={message.id}
                        className={`flex gap-3 px-1 ${isStaff ? 'flex-row-reverse' : 'flex-row'}`}
                      >
                        {/* Avatar */}
                        <div className="flex-shrink-0 mt-auto mb-1">
                          {isStaff ? (
                            <div className="w-8 h-8 rounded-full bg-blue-50 flex items-center justify-center border border-blue-100 overflow-hidden shadow-sm">
                               <span className="text-[10px] font-bold text-blue-600">{(user?.name || 'S').charAt(0)}</span>
                            </div>
                          ) : (
                            <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center border border-gray-200 overflow-hidden shadow-sm">
                              {conversation.customer?.avatarUrl ? (
                                <img src={conversation.customer.avatarUrl} alt="" className="w-full h-full object-cover" />
                              ) : (
                                <span className="text-[10px] font-bold text-gray-500">{customerInitial}</span>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div className={`flex flex-col max-w-[75%] ${isStaff ? 'items-end' : 'items-start'}`}>
                          ...
                          <div className={`flex items-center gap-2 mb-1 px-1 text-[10px] font-medium text-gray-400 ${isStaff ? 'flex-row-reverse' : 'flex-row'}`}>
                            <span className="text-gray-600">{senderName}</span>
                          ...
                          {/* Bubble */}
                          <div
                            className={`relative group rounded-2xl px-4 py-2.5 transition-all duration-300 shadow-sm ${
                              isStaff 
                                ? 'bg-blue-600 text-white rounded-br-sm hover:bg-blue-700' 
                                : 'bg-white text-gray-800 rounded-bl-sm border border-gray-200/60 hover:border-gray-300 shadow-gray-200/50'
                            }`}
                          >
            ...
            <div className="border-t border-gray-200 px-8 py-5 bg-white">
              <div className="max-w-3xl mx-auto space-y-2">
                ...
                {isInactiveConversation ? (
                  <div className="flex flex-col items-center justify-center py-10 px-6 rounded-3xl bg-gray-50/80 border border-dashed border-gray-300 animate-in fade-in slide-in-from-bottom-4 duration-500">
                    <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center mb-4 text-gray-400 shadow-sm border border-gray-100">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                      </svg>
                    </div>
                    <h3 className="text-base font-bold text-gray-900 uppercase tracking-widest">Conversation Closed</h3>
                    <p className="text-sm text-gray-500 mt-2 text-center max-w-sm">
                      This conversation has been marked as {isResolved ? 'resolved' : 'closed'}. 
                      The input field is disabled to maintain response integrity.
                    </p>
                  </div>
                ) : !canSendMessages ? (
                  <div className="flex flex-col items-center justify-center py-10 px-6 rounded-3xl bg-blue-50/30 border border-dashed border-blue-200">
                    <div className="w-12 h-12 rounded-full bg-white flex items-center justify-center mb-4 text-blue-400 shadow-sm border border-blue-100">
                      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                      </svg>
                    </div>
                    <h3 className="text-base font-bold text-blue-900 uppercase tracking-widest">Restricted Access</h3>
                    <p className="text-sm text-blue-700/70 mt-2 text-center max-w-sm">
                      {isAssigned
                        ? `This conversation is assigned to ${assignedAgentName}. Only they can reply.`
                        : 'This conversation is currently unassigned. Please assign it to yourself or an agent to reply.'}
                    </p>
                  </div>
                ) : null}
