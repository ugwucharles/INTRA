      ...
      <div className="max-w-2xl mx-auto space-y-1">
        {messages.length === 0 ? (
          ...
          messages.map((message, index) => {
            const isNote = (message as any).isNote === true;
            const isStaff = message.senderType === 'STAFF';
            const isBot = isStaff && !message.senderId && !isNote;
            const senderName = isNote
              ? ((message as any).authorName || staff.find((s) => s.id === message.senderId)?.name || 'Agent')
              : isBot
                ? 'Automatic Reply'
                : isStaff
            ...
            const sameSender = prevMessage?.senderType === message.senderType && (prevMessage?.senderId === message.senderId) && !!(prevMessage as any).isNote === !!isNote;
            const showHeader = !sameSender;

            // Private notes get full-width amber treatment (not right-aligned)
            if (isNote) {
              return (
                <div key={message.id} className={`${showHeader ? 'mt-4' : 'mt-0.5'}`}>
                  {showHeader && (
                    <div className="flex items-center gap-1.5 mb-1 px-1">
                      ...
                      <span className="text-[11px] font-medium text-amber-700">{senderName}</span>
                      <span className="px-1.5 py-0.5 text-[9px] font-semibold bg-amber-100 text-amber-700 border border-amber-300 rounded uppercase tracking-wider">
                        Private note
                      </span>
                  ...
                  <div className="ml-9 rounded-xl px-3.5 py-2.5 bg-amber-50 border border-amber-200 border-dashed text-amber-900">
