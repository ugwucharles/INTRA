...
import { ConversationStatus, FallbackBehavior, RouteTo, UserRole } from '@prisma/client';
...
export class RoutingService {
   ...
   */
  async handleInboundRouting(params: RoutingParams): Promise<RoutingOutcome | null> {
    ...
    const existingMetadata: any =
      conversation.routingMetadata && typeof conversation.routingMetadata === 'object'
  ...
  private async autoAssignConversation(
    ...
    if (agents.length === 0 && routeTo === RouteTo.LIVE_AGENTS_ONLY && fallback === FallbackBehavior.ASSIGN_ANY_AGENT) {
