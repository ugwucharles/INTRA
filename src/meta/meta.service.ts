...
export class MetaService {
  ...
  async handleWebhook(body: any) {
    ...
    for (const entry of body.entry) {
      ...
      if (
        ...
        for (const change of entry.changes) {
          if (change.field === 'messages' && change.value) {
            ...
            if (phoneNumberId) {
              const creds =
                await this.socialAccounts.findOrgByPhoneNumberId(phoneNumberId);
            ...
            try {
              ...
              this.logger.error(
                'Failed to handle WhatsApp webhook value',
                err as Error,
              );
            }
          }
      ...
      if (!orgId) {
        this.logger.error(
          `CRITICAL: No active SocialAccount for pageId=${pageId ?? 'unknown'} (Object: ${body.object}); skipping entry.`,
        );
        continue;
      }
      ...
      for (const event of messagingEvents) {
        try {
          ...
          this.logger.error(
            `Failed to handle messaging event: ${err.message}`,
            err as Error,
          );
        }
      }
  ...
  async syncCustomerProfile(orgId: string, customerId: string) {
    const customer = await this.prisma.customer.findFirst({
      where: { id: customerId, orgId },
    });
    if (!customer || !customer.externalId) return null;

    let profile: { name: string | null; avatarUrl: string | null } = {
      name: null,
      avatarUrl: null,
    };
    if (customer.source === $Enums.Channel.FACEBOOK_MESSENGER) {
      profile = await this.fetchMessengerProfile(
        orgId,
        customer.externalId,
        customer.pageId || undefined,
      );
    } else if (customer.source === $Enums.Channel.INSTAGRAM) {
      profile = await this.fetchInstagramProfile(
        orgId,
        customer.externalId,
        customer.pageId || undefined,
      );
  ...
  /**
   * Fetch the profile (name and avatar) for a Facebook Messenger user (PSID) via Graph API.
   */
  private async fetchMessengerProfile(
    ...
    const creds = await this.socialAccounts.findCredentials(
      orgId,
  ...
  /**
   * Fetch the profile (name and avatar) for an Instagram sender via Graph API.
   */
  private async fetchInstagramProfile(
    ...
    try {
      ...
      this.logger.error('Failed to fetch Instagram profile', err as Error);
      return { name: null, avatarUrl: null };
    }
  }
  ...
  private async sendWhatsAppTextMessage(
    ...
    const waCreds = await this.socialAccounts.findCredentials(
      orgId,
   ...
   */
  private async handleWhatsAppWebhookValue(
    orgId: string,
    value: any,
  ): Promise<void> {
    ...
    for (const msg of messages) {
      ...
      if (msg.type !== 'text' || !msg.text?.body) {
        this.logger.log(
          `Ignoring non-text WhatsApp message (type=${msg.type})`,
        );
        continue;
      }
      ...
      // Routing
      try {
        const routingOutcome: RoutingOutcome | null =
          hadRoutingQuestionAlreadySent
  ...
  private async handleMessagingEvent(
    ...
    if (channel === $Enums.Channel.FACEBOOK_MESSENGER) {
      profile = await this.fetchMessengerProfile(orgId, senderId, pageId);
    } else if (channel === $Enums.Channel.INSTAGRAM) {
      profile = await this.fetchInstagramProfile(orgId, senderId, pageId);
    }
