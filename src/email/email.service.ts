...
export class EmailService implements OnModuleInit, OnModuleDestroy {
  ...
  private async initImap() {
    ...
    while (true) {
      ...
      try {
        ...
        const existsHandler = (data: { count: number }) => this.handleNewEmailCount(data.count);
        ...
        let pollingInterval: ReturnType<typeof setInterval> | null = setInterval(() => {
    ...
    try {
      ...
      for await (const message of fetcher) {
        if (message.source && message.uid && message.uid > this.lastProcessedUID) {
          toProcess.push({ uid: message.uid, source: message.source as Buffer });
        }
      }
    ...
    try {
      ...
      const latest = await this.prisma.message.findFirst({
        where: {
          ...
          externalId: { not: null },
          conversation: {
            customer: { source: 'EMAIL' as any, orgId },
          },
        },
   ...
   */
  async sendReply(orgId: string, conversationId: string, toEmail: string, text: string, messageId: string) {
